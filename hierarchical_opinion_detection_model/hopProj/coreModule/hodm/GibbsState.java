...
public class GibbsState {
	...
	private final int output_lag=1000;//吉布斯采样中每隔output_lag行输出到文件中,是个常量
	...
	public GibbsState(String corpusFileName,String settingsFileName)
	{
		...
		try{
			...
			eta=new Vector<Double>(depth);//读取ETA//为什么eta是一个与层有关的标量呢?正常情况下应该是向量,或者是与层有关的nkeys维的向量的啊,或者说,depth*nkeys,是这样子,这里作者认为topic是关于eta的symmertic分布
			...
			this.hodmTree=new HODMTree(depth,this.corpus.nkeys,eta,gamma);
			this.run_dir=null;
	...
	public void initGibbsState()
	{
		...
		for(i=0;i<this.corpus.ndocs;++i)
		{
			...
			Collections.shuffle(doc_i.phrases);//把phrase的语序打乱
			Topic buttomChildNode=this.hodmTree.priorTreeFillNodeInInitState(this.hodmTree.root,this.corpus.nkeys,this.eta);//在获得最底端的孩子结点的同时,整个树的一条path已经产生
	...
	public void writeGibbsState(String out_filename)
	{
		...
		try{
			File treefile=new File(out_filename);//这个file是用来输出tree信息的,也包括state的信息
			FileOutputStream treefos=new FileOutputStream(treefile);
			BufferedWriter treebw=new BufferedWriter(new OutputStreamWriter(treefos,"utf-8"));
			
			String alineToWrite;//作为输出的aline
			
			alineToWrite=String.format("%s %17.14e\n","SCORE",score);//输出double类型的score
			treebw.write(alineToWrite);
			alineToWrite=String.format("%s %d\n","ITER",this.iter);//输出int类型的迭代次数
			treebw.write(alineToWrite);
			alineToWrite=String.format("%-10s","ETA");//输出向量eta
			for(int i=0;i<eta.size();++i)
			{
				alineToWrite+=String.format(" %17.14e",this.eta.get(i));
			}
			alineToWrite+="\n";
			treebw.write(alineToWrite);
			alineToWrite=String.format("%-10s","GAMMA");//输出向量gamma,其维度为DEPTH-1,其实gamma并没有从第二层开始逐层变化,只是按照这种格式输出而已
			for(int i=0;i<this.hodmTree.depth-1;++i)
			{
				alineToWrite+=String.format(" %17.14e",this.hodmTree.gamma);
			}
			alineToWrite+="\n";
			treebw.write(alineToWrite);
			alineToWrite=String.format("%s %17.14e\n","GEM_MEAN",0.0);
			...
			alineToWrite=String.format("%s %17.14e\n","SCALING_SHAPE",1.0);
			treebw.write(alineToWrite);
			alineToWrite=String.format("%s %17.14e\n","SCALING_SCALE",this.hodmTree.gamma);
			treebw.write(alineToWrite);
			
			this.hodmTree.writeTree(treebw);
			treebw.close();
			treefos.close();
